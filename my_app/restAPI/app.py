...
def get_children():
    ...
    children_list = [{'ID': row[0], 'FamilyID': row[1], 'Name': row[2], 'DOB': row[3].strftime('%Y-%m-%d'), 'Sex': row[4], 'Country': row[5], 'LevelOfEducation': row[6]} for row in children]
    conn.close()
...
def add_child():
    ...
    VALUES (?, ?, ?, ?, ?, ?);
    '''
    cursor.execute(query, (new_child['FamilyID'], new_child['Name'], new_child['DOB'], new_child['Sex'], new_child['Country'], new_child['LevelOfEducation']))
    conn.commit()
    conn.close()
